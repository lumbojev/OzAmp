...
setlocal
cd /d "%~dp0"
del /q main.obj audio_engine.obj OzAmp-1.0.2.exe 2>nul
...
rem Regenerate every build; stale resources must never lose the manifest or version.
where python >nul 2>nul || (echo ERROR: Python 3 is required to build application resources. & exit /b 1)
python tools\make_icon_res.py ozamp.ico ozamp.res ozamp.manifest main.cpp || exit /b 1
